...
<div class="min-h-dvh bg-bg text-text flex flex-col max-w-md mx-auto">
	...
	<header class="felt px-6 pt-8 pb-6 flex flex-col gap-3 header-in">
		<p class="text-text-muted text-sm tracking-widest" aria-hidden="true">♠ ♥ ♦ ♣</p>
		<h1 class="font-display text-6xl tracking-wider leading-none">Leaderboard</h1>
		{#if data.entries.length > 0}
			<p class="text-text-muted text-sm">
				{data.entries.length} player{data.entries.length === 1 ? '' : 's'} · all-time
			</p>
		{/if}
	</header>
	...
	<div class="flex-1 flex flex-col gap-6 p-5 pb-10">
		...
		{#if data.entries.length === 0}
			...
			<div class="flex flex-col items-center justify-center gap-3 py-16 text-center">
				...
				<p class="text-text-muted text-sm">No completed sessions yet.</p>
				<p class="text-text-muted text-xs">Close a session to see stats here.</p>
			</div>
		{:else}
					...
					{#each data.entries as entry, i (entry.playerId)}
						...
						>
							...
							<div class="flex flex-col gap-0.5 min-w-0 flex-1">
								<span class="font-semibold text-text truncate {i === 0 ? 'text-gold-light' : ''}">
									{entry.playerName}
								</span>
							...
							<!-- Total net -->
							<span class={['tabular font-semibold text-lg shrink-0', getNetClass(entry.totalNet)]}>
			...
			<div class="flex items-center gap-3">
				...
				<span class="text-text-muted text-base leading-none" aria-hidden="true">♥ ♠ ♦ ♣</span>
				<div class="flex-1 border-t border-border"></div>
			</div>
...
<style>
	...
	.player-row {
		opacity: 0;
		animation: cascade-in 320ms var(--ease-out-back, cubic-bezier(0.34, 1.56, 0.64, 1)) forwards;
	}

	/* Gold row — subtle highlight for #1 */
	.gold-row {
		background-color: color-mix(in srgb, var(--color-gold-dim) 25%, transparent);
	}
	.gold-row:hover {
		background-color: color-mix(in srgb, var(--color-gold-dim) 35%, transparent);
	}
</style>
